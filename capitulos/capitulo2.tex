...
Atraves da analise de sistemas de teleassitencia, utilizados no mercado actualmente, foi possivel estudar o problema em questao, identificando os funcionalidades mais relevantes e os pontos fracos dos sistemas estudos. Na \ref{fig:cenario_global} e apresentada uma visão global do sistema na qual se podem identificar as principais funcionalidades e entidades que interagem com o sistema, assim sendo e possível desta forma apresentar de uma forma simples a interacção entre os actores e o sistema.

\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.5\textwidth]{figuras/cenario_global_utilizacao.jpg}
...
\begin{itemize}
	\item\textbf{Adicionar de um piquete -} O gestor do sistema deve criar um, ou mais, entidades que recebem os avisos de ocorrências (piquete) por parte do sistema. A esta entidade deve estar associado uma séria de informações que garantam a capacidade de contacto. 
...
\begin{itemize}
	\item\textbf{Gestor do sistema - }Tem como função gerir e  supervisionar o sistema, garantindo a gestão dos piquetes, gestão das ocorrências e gestão das novidades, através de operacoes CRUDE.
...
Para descrever os principais casos de uso do sistema, utilizou-se o diagrama de casos de uso presente na \ref{fig:diagrama_casos_uso}. Podemos observar nesta figura a interacção entre elementos do sistema num contexto de elevada abstracção.
Os principais casos descritos no âmbito deste trabalho sao:
\begin{itemize}
	\item Adicionar piquete por parte do actor 2
	\item Utilização do botão SOS por parte do actor 1 
	\item Detecao de queda por parte do actor 1
...
\\\\
\textbf{Geolocalizacao:}o sistema deve ser capaz de saber a posição actual e enviá-la quando necessário (queda, botão, imobilização, …).
\\\\
\textbf{Acelerómetro e giroscópio:}o sistema tem de ter sensores que lhe permitam detectar uma queda.
\\\\
...
\\\\
\textbf{Preço:}O sistema deve ter um custo inicial de desenvolvimento inferior a 25000\euro, um custo de produção de hardware (dispositivo wereable) inferior a 100\euro e um custo de manutenção anual inferior a 50000\euro para o serviço de teleassistência.
\\\\
...
\\\\
\textbf{Implementação de base de dados relacional:} deve ser possivel permitir a evolução futuro do sistema, permitindo uma gestao mais eficiente dos equipamentos.